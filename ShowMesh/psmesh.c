...
**********************/
 ...
 if(butt_data[MATERIALS].pressed==ON || butt_data[ELEMENTS].pressed==ON)
   for(e=0; e<Ne; e++)
	...
	sprintf(numb, "%d", elem[e].mark);
     	printf("%d %d moveto\n",(int)(x*scl + X0),(int)(-y*scl + Y0));
	printf("(%d) show\n",elem[e].mark);
     }
     if(butt_data[ELEMENTS].pressed ==ON) {
	sprintf(numb, "%d", e);
     	printf("%d %d moveto\n",(int)(x*scl + X0),(int)(-y*scl + Y0));
	printf("(%d) show\n",e);
     }
...
+----------------------------*/
 ...
 X0 = (int)(draw_wdth*.05) - (int)(xmin*scl);
 Y0 = draw_hght-(int)(draw_hght*0.05) +(int)(ymin*scl);
