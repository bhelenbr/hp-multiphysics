...
void tet_hp_ins::length() {
		...
		for(int i=0;i<lgpx;++i) {
			for(int j=0;j<lgpy;++j) {
				for(int k=0;k<lgpz;++k) {
					...
					bernoulli = hp_ins_gbl->rho*(u(0)(i)(j)(k)*u(0)(i)(j)(k) +u(1)(i)(j)(k)*u(1)(i)(j)(k)+u(2)(i)(j)(k)*u(2)(i)(j)(k)) +u(NV-1)(i)(j)(k);	
					/* VISCOUS PART TO ERROR MEASURE */
					bernoulli += hp_ins_gbl->mu*(fabs(dudx)+fabs(dudy)+fabs(dudz)+fabs(dvdx)+fabs(dvdy)+fabs(dvdz)+fabs(dwdx)+fabs(dwdy)+fabs(dwdz))/jcb;
					
					dbernoulli = hp_ins_gbl->rho*(ul(0)(i)(j)(k)*ul(0)(i)(j)(k) +ul(1)(i)(j)(k)*ul(1)(i)(j)(k)+ul(2)(i)(j)(k)*ul(2)(i)(j)(k)) +ul(NV-1)(i)(j)(k);	
					dbernoulli += hp_ins_gbl->mu*(fabs(dudxl)+fabs(dudyl)+fabs(dudzl)+fabs(dvdxl)+fabs(dvdyl)+fabs(dvdzl)+fabs(dwdxl)+fabs(dwdyl)+fabs(dwdzl))/jcb;
	...
	hp_ins_gbl->eanda(0) = totalbernoulli;
	hp_ins_gbl->eanda(1) = denom;
	hp_ins_gbl->eanda(2) = totalerror;
	sim::blks.allreduce(hp_ins_gbl->eanda.data(),hp_ins_gbl->eanda_recv.data(),3,blocks::flt_msg,blocks::sum);
	
	/* Determine error target (SEE AEA Paper) */
	FLT etarget2 = gbl->error_target*gbl->error_target*hp_ins_gbl->eanda_recv(0);
	*gbl->log << "# Normalized Error " << sqrt(hp_ins_gbl->eanda_recv(2)/hp_ins_gbl->eanda_recv(0)) << " Target " << gbl->error_target << '\n';
