...
gmres( int m, int n, const Matrix &A, const double *b, double *x, double eps,
  ...
  int its=-1;
  {
	...
	u0j=uij;
	mult(A,v[j],v[j+1]);
	cblas_dgemv(CblasRowMajor,CblasTrans,n,j+1,1.,V,n,v[j+1],1,0.,U+u0j,1);
	cblas_dgemv(CblasRowMajor,CblasNoTrans,n,j+1,-1.,V,n,U+u0j,1,1.,v[j+1],1);
	...
	if ( detailed )
      ...
      { // minimiere bzgl Y
	cblas_dtpsv(CblasRowMajor,CblasUpper,CblasNoTrans,CblasNonUnit,j,U,y,1);
	// korrigiere X
	cblas_dgemv(CblasRowMajor,CblasNoTrans,n,j,-1.,V,n,y,1,1.,x,1);
      }
