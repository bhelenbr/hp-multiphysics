...
def getdata():
	dof0 = int(cut_fields(grep_last('output_b0.log', "DOF:"), [6]))
	dof1 = int(cut_fields(grep_last('output_b1.log', "DOF:"), [6]))
	dof = dof0 + dof1
	...
	viscous6 = cut_fields(grep_last('output_b1.log','b1_s6 viscous/pressure',1), [2,3,4])
	total6 = cut_fields(grep_last('output_b1.log','b1_s6 total fluxes',1), [2,3,4])
	viscous8 = cut_fields(grep_last('output_b1.log','b1_s8 viscous/pressure',1), [2,3,4])
	total8 = cut_fields(grep_last('output_b1.log','b1_s8 total fluxes',1), [2,3,4])
	
	with open('cnvg.dat', "a") as out:
		out.write(f"{dof} {viscous1} {viscous6} {viscous8} {total1} {total6} {total8}\n")
...
for log2p in range(3):
	...
	run(["mpiexec", "-np", "2", "tri_hp_petsc", "run.inpt"])
	getdata()
	...
	# Process the data
	forceData = np.loadtxt("cnvg.dat", delimiter=' ')
	viscPres = forceData[:,1:4]+forceData[:,4:7]+forceData[:,7:10]
	totalFlux = forceData[:,10:13]+forceData[:,13:16]+forceData[:,16:19]
	output = np.hstack([forceData[:,0:1],viscPres,totalFlux])
	np.savetxt("cnvg2.dat",output,delimiter=' ',fmt='%.8e',header='DOF,viscPresX viscPresY viscPresZ totalFluxX totalFluxY totalFluxZ')
	os.chdir("..")
