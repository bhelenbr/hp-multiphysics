...
block::ctrl tri_hp_ins::rsdl(int excpt, int stage) {
   
   if (excpt > 0) {
      *sim::log << "flow control error in residual" << std::endl;
      return(block::stop);
   }
   ...
   FLT rhobd0 = ins_gbl->rho*sim::bd[0], lmu = ins_gbl->mu, rhorbd0, lrhorbd0, cjcb, cjcbi;
      ...
      if (td(tind).info > -1) {
         ...
         for(i=0;i<lgpx;++i) {
            for(j=0;j<lgpn;++j) {
               ...
               fluxx = ins_gbl->rho*RAD(i,j)*(u(0)(i,j) -mvel(0)(i,j));
               fluxy = ins_gbl->rho*RAD(i,j)*(u(1)(i,j) -mvel(1)(i,j));
...
#ifdef BODY
                  res(0)(i,j) -= ins_gbl->rho*RAD(i,j)*cjcb*body[0];
                  res(1)(i,j) -= ins_gbl->rho*RAD(i,j)*cjcb*body[1];
#ifdef INERTIALESS
                  res(0)(i,j) = -ins_gbl->rho*RAD(i,j)*cjcb*body[0];
                  res(1)(i,j) = -ins_gbl->rho*RAD(i,j)*cjcb*body[1];
#endif
...
//      std::cout << ins_gbl->res.v;
//      std::cout << ins_gbl->res.s;
//      std::cout << ins_gbl->res.i;
//      exit(1);
