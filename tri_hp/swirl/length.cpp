...
void tri_hp_swirl::length() {
	...
	for(tind=0;tind<ntri;++tind) {
		...
		hp_gbl->eanda(0) += 1./3.*( (0.5*hp_ins_gbl->rho*q +p)*area(tind) +duv*hp_ins_gbl->mu*sqrt(area(tind)) );
	...
	switch(basis::tri(log2p)->p()) {
		case(1): {
			for(i=0;i<nseg;++i) {
				...
				ruv = hp_ins_gbl->rho*0.5*(u + v + w) +hp_ins_gbl->mu/distance(v0,v1);
