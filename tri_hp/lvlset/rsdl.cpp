		...
		for(i=0;i<lgpx;++i) {
			for(j=0;j<lgpn;++j) {
				...
				if (u(2)(i,j) < -gbl->width) {
					...
					tang /= length;
					rho(i,j) = gbl->rho +(gbl->rho2 -gbl->rho)*heavy;
					mu(i,j) = gbl->mu +(gbl->mu2 -gbl->mu)*heavy;
					...
					cv(0,0)(i,j) += -delt*gbl->sigma*tang(0)*length*(dcrd(1,1)(i,j)*tang(0) -dcrd(0,1)(i,j)*tang(1));
					cv(0,1)(i,j) += +delt*gbl->sigma*tang(0)*length*(dcrd(1,0)(i,j)*tang(0) -dcrd(0,0)(i,j)*tang(1));
					cv(1,0)(i,j) += -delt*gbl->sigma*tang(1)*length*(dcrd(1,1)(i,j)*tang(0) -dcrd(0,1)(i,j)*tang(1));
					cv(1,1)(i,j) += +delt*gbl->sigma*tang(1)*length*(dcrd(1,0)(i,j)*tang(0) -dcrd(0,0)(i,j)*tang(1));
				}
...
#ifdef CONSERVATIVE
		basis::tri(log2p)->intgrtrs(&lf_im(NV-2)(0),&cv(NV-2,0)(0,0),&cv(NV-2,1)(0,0),MXGP);
#else
...
#define                      viscI1II0II1II0I visc(0,1)(0,1)                                
			...
			basis::tri(log2p)->intgrt(&lf_re(NV-1)(0),&res(NV-1)(0,0),MXGP);
#ifdef CONSERVATIVE
			basis::tri(log2p)->intgrt(&lf_re(NV-2)(0),&res(NV-2)(0,0),MXGP);
#else
		...
		for(i=0;i<lgpx;++i) {
			for(j=0;j<lgpn;++j) {
				...
				if (phidw < -1.0) {
					...
					res(2)(i,j) = (1.-length)*cjcb*RAD(crd(0)(i,j));
...
#ifdef LOCALIZED_WITH_DISTANCE_FUNCTION
					res(2)(i,j) = (length-1.0)*cjcb*RAD(crd(0)(i,j));
					phivel(0)(i,j) = norm(0)/length;
					phivel(1)(i,j) = norm(1)/length;
#elif defined(NONCONSERVATIVE)
					res(2)(i,j) = RAD(crd(0)(i,j))*cjcb*(gbl->bd(0)*u(NV-2)(i,j) +dugdt(log2p,tind,NV-2)(i,j)
					...
					/* SURFACE TENSION TERMS */
					cv(0,0)(i,j) += -delt*gbl->sigma*norm(1)*length*(ldcrd(1,1)*norm(1) +ldcrd(0,1)*norm(0));
					cv(0,1)(i,j) += +delt*gbl->sigma*norm(1)*length*(ldcrd(1,0)*norm(1) +ldcrd(0,0)*norm(0));
					cv(1,0)(i,j) += +delt*gbl->sigma*norm(0)*length*(ldcrd(1,1)*norm(1) +ldcrd(0,1)*norm(0));
