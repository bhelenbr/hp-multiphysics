...
void tri_hp_ps::element_rsdl(int tind, int stage, Array<TinyVector<FLT,MXTM>,1> &uht,Array<TinyVector<FLT,MXTM>,1> &lf_re,Array<TinyVector<FLT,MXTM>,1> &lf_im) {
	int i,j,n;
	FLT fluxx,fluxy;
	const int NV = 3;
	TinyVector<int,3> v;
    ...
    if (tri(tind).info > -1) {
        ...
        for(i=0;i<lgpx;++i) {
            for(j=0;j<lgpn;++j) {
                ...
                fluxx = RAD(crd(0)(i,j))*u(0)(i,j);
                fluxy = RAD(crd(0)(i,j))*u(1)(i,j);
