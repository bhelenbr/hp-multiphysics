...
void tri_hp_cns::project_new_variables(){
	...
	hp_cns_gbl->res_temp.v = hp_gbl->res.v;
	hp_cns_gbl->res_temp.s = hp_gbl->res.s;
	hp_cns_gbl->res_temp.i = hp_gbl->res.i;
	...
	for(int i=0;i<npnt;++i){
		...
		for(int j=0;j<NV;++j){
			FLT lcl0 = lclres(j);
			for(int k=0;k<j;++k){
				lcl0 -= hp_cns_gbl->vpreconditioner(i,j,k)*lclres(k);
			}
			lclres(j) = lcl0/hp_cns_gbl->vpreconditioner(i,j,j);
		}
		
		for(int n = 0; n < NV; ++n)
			hp_gbl->res.v(i,n) = lclres(n);
    ...
    for(int sind=0;sind<nseg;++sind) {
		...
		/* project linears */
		for(int n=0;n<NV;++n)
			basis::tri(log2p)->proj1d(hp_gbl->res.v(seg(sind).pnt(0),n),hp_gbl->res.v(seg(sind).pnt(1),n),&temp1d(n)(0));
		
		for(int m=0;m<NV;++m) 
			for(int n=0;n<NV;++n) 
				basis::tri(log2p)->proj1d(hp_cns_gbl->vpreconditioner(seg(sind).pnt(0),m,n),hp_cns_gbl->vpreconditioner(seg(sind).pnt(1),m,n),&P(m,n)(0));
		
		/* take global coefficients and put into local vector */
		for(int n=0;n<NV;++n) {
			for (int m=0; m<2; ++m) {
				ucoef(n)(m) = ug.v(seg(sind).pnt(m),n);
				rcoef(n)(m) = hp_cns_gbl->res_temp.v(seg(sind).pnt(m),n);
			}
		}
		...
		for(int n=0;n<NV;++n) {
			for(int m=0;m<basis::tri(log2p)->sm();++m) {
				...
				rcoef(n)(m+2) = hp_cns_gbl->res_temp.s(sind,m,n);
			}
		}
...
#endif
			for(int m=0;m<basis::tri(log2p)->sm();++m) 
				hp_gbl->res.s(sind,m,n) = -rcoef(n)(m+2);
	...
	for(int tind = 0; tind < ntri; ++tind) {
		...
		for (int i=0; i<3; ++i) {
			...
			for(int n=0; n<NV; ++n){
				...
				rcoef(n)(i) = hp_cns_gbl->res_temp.v(vrtx,n);
				tcoef(n)(i) = hp_gbl->res.v(vrtx,n);
			}
		}
		...
		for(int i=0;i<3;++i) {
			...
			for (int m = 0; m < basis::tri(log2p)->sm(); ++m) {
				for(int n=0; n<NV; ++n){
					...
					rcoef(n)(cnt) = msgn*hp_cns_gbl->res_temp.s(sind,m,n);
					tcoef(n)(cnt) = msgn*hp_gbl->res.s(sind,m,n);
				}
				msgn *= sign;
				++cnt;
			}
			...
			for(int m = 1; m < basis::tri(log2p)->sm(); ++m) {
				for(int k = 0; k < basis::tri(log2p)->sm()-m; ++k) {
					for(int n=0; n<NV; ++n){
						...
						rcoef(n)(cnt) = hp_cns_gbl->res_temp.i(tind,indx,n);
						tcoef(n)(cnt) = 0.0;
					}
					++cnt; ++indx;
				}
		...
		for(int m=0;m<NV;++m)		
			for(int n=0;n<NV;++n)
				basis::tri(log2p)->proj(hp_cns_gbl->vpreconditioner(tri(tind).pnt(0),m,n),hp_cns_gbl->vpreconditioner(tri(tind).pnt(1),m,n),hp_cns_gbl->vpreconditioner(tri(tind).pnt(2),m,n),&P2d(m,n)(0,0),MXGP);
		...
		for(int n=0;n<NV;++n) {
			...
			for(int i=0;i<basis::tri(log2p)->im();++i)
				hp_gbl->res.i(tind,i,n) = -rcoef(n)(basis::tri(log2p)->bm()+i);
		}
	}
...
void tri_hp_cns::switch_variables(Array<double,1> pvu, Array<double,1> &a){
	...
	double gm1 = hp_cns_gbl->gamma-1.0;
...
void tri_hp_cns::project_res_side(int mode) {
    ...
    for(int sind=0;sind<nseg;++sind) {
		...
		for(int n=0;n<NV;++n) {
			for(int m=0;m<basis::tri(log2p)->sm();++m) {
				ucoef(n)(m+2) = hp_gbl->ug0.s(sind,m,n);
				rcoef(n)(m+2) = hp_cns_gbl->res_temp.s(sind,m,n);
			}
		}
		...
		for(int n=0;n<NV;++n) {
			...
			//			for(int m=0;m<basis::tri(log2p)->sm();++m) 
			//				hp_gbl->res.s(sind,m,n) = -rcoef(n)(m+2);
			hp_gbl->res.s(sind,mode,n) = -rcoef(n)(mode+2);
...
//		for(i=0;i<npnt;++i) {
//
//			Pinv = hp_gbl->vprcn_ut(i,Range::all(),Range::all());
//			temp = hp_gbl->res.v(i,Range::all());
//
//			int info,ipiv[NV];
...
//			}
//
//			hp_gbl->res.v(i,Range::all()) = temp/basis::tri(log2p)->vdiag();
//		}
//	}
//	
//	hp_cns_gbl->res_temp.v = hp_gbl->res.v;
...
//	
//	if (hp_gbl->diagonal_preconditioner) {  // IF STATEMENT IN LOOP IS BAD FIXME
//		for(tind=0;tind<ntri;++tind) {
//			for(i=0;i<3;++i) {
//				v0 = tri(tind).pnt(i);
//				for(n=0;n<NV;++n)
...
//			for(sind = 0; sind < nseg; ++sind) {
//								
//				Pinv = hp_gbl->sprcn_ut(sind,Range::all(),Range::all());
//				temp = hp_gbl->res.s(sind,mode,Range::all());
//				
//				int info,ipiv[NV];
//				GETRF(NV, NV, Pinv.data(), NV, ipiv, info);
//				
...
//				}
//				
//				hp_gbl->res.s(sind,mode,Range::all()) = temp/basis::tri(log2p)->sdiag(mode);
//				
//			}
//		}
//		
...
//				
//				Pinv = hp_gbl->tprcn_ut(tind,Range::all(),Range::all());
//				temp = hp_gbl->res.i(tind,k,Range::all());
//				
//				int info,ipiv[NV];
//				GETRF(NV, NV, Pinv.data(), NV, ipiv, info);
//				
...
//				}
//				
//				hp_gbl->res.i(tind,k,Range::all()) = temp;
//			}
//		}
//	}
//
////	if(hp_gbl->diagonal_preconditioner){
////		hp_cns_gbl->res_temp.s = hp_gbl->res.s; //not sure if i need this one
////		hp_cns_gbl->res_temp.i = hp_gbl->res.i;
////		
