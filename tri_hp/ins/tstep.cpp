...
block::ctrl tri_hp_ins::setup_preconditioner(block::ctrl ctrl_message) {
   ...
   if (excpt == 3) {
      ...
      /** DETERMINE FLOW PSEUDO-TIME STEP ****/
      /***************************************/
      gbl_ptr->vprcn(Range(0,nvrtx-1),Range::all(),Range::all()) = 0.0;
      if (basis::tri(log2p).sm > 0) {
         gbl_ptr->sprcn(Range(0,nside-1),Range::all(),Range::all()) = 0.0;
      }
      gbl_ptr->tprcn(Range(0,ntri-1),Range::all(),Range::all()) = 0.0;
      ...
      for(tind = 0; tind < ntri; ++tind) {
         ...
         gbl_ptr->tprcn(tind,0,0) = gbl_ptr->rho*jcb;   
         gbl_ptr->tprcn(tind,1,1) = gbl_ptr->rho*jcb;     
         gbl_ptr->tprcn(tind,2,2) = jcb/gam;
         gbl_ptr->tprcn(tind,0,2) = -jcb*ubar/gam;
         gbl_ptr->tprcn(tind,1,2) = -jcb*vbar/gam;
         for(i=0;i<3;++i) {
            gbl_ptr->vprcn(v(i),Range::all(),Range::all())  += gbl_ptr->tprcn(tind,Range::all(),Range::all());
            if (basis::tri(log2p).sm > 0) {
               side = td(tind).side(i);
               gbl_ptr->sprcn(side,Range::all(),Range::all()) += gbl_ptr->tprcn(tind,Range::all(),Range::all());
            }
         }
      }
   }
