...
void hp_coupled_bdry::setup_preconditioner() {
	
	hp_edge_bdry::setup_preconditioner();
#ifdef petsc
	/* Shut off preconditioning caused by r_tri_mesh */
	int i = 0;
	int sind;
	do {
		sind = base.seg(i);
...
int hp_coupled_bdry::non_sparse_rcv(Array<int, 1> &nnzero, Array<int, 1> &nnzero_mpi) {
	...
	if (sm) {
		for (int i=base.nseg-1;i>=0;--i) {
			int Jcount = jacobian_start +i*sm*NV;
			for (int mode=0;mode<sm;++mode) {
				for(int n=0;n<NV;++n) {
					target(Jcount++) += base.ircvbuf(0,count++);
				}
			}
		}
	}
...
void hp_coupled_bdry::petsc_matchjacobian_snd() {
	// master only sends indices of continuous variables
	// slave send only information for flow not mesh variables
...
int hp_coupled_bdry::petsc_matchjacobian_rcv(int phase) {
	...
	hp_edge_bdry::petsc_matchjacobian_rcv(phase);
...
int hp_coupled_bdry::petsc_matchjacobian_rcv(int phase) {
	...
	int count = hp_edge_bdry::petsc_matchjacobian_rcv(phase);
