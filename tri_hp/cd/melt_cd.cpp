...
void melt_cd::non_sparse(Array<int,1> &nnzero) {
	...
	hp_coupled_bdry::non_sparse(nnzero);
...
int melt_cd::non_sparse_rcv(Array<int, 1> &nnzero, Array<int, 1> &nnzero_mpi) {
	int count = hp_coupled_bdry::non_sparse_rcv(nnzero,nnzero_mpi);
...
void melt_cd::petsc_make_1D_rsdl_vector(Array<double,1> res) {
	...
	const int vdofs = x.NV +(x.mmovement == tri_hp::coupled_deformable)*x.ND;
	Array<FLT,1> res_vec(3);

#ifdef SYMMETRIC
	for(int j=0;j<base.nseg;++j) {
		int ind1 = x.npnt*vdofs +base.seg(j)*sm*x.NV +c0_indices[0];  // Index of heat equation
		for(int m=0;m<sm;++m) {
			res_vec(0) = res(ind1);
			res_vec(1) = gbl->sres(j,m,0);
			res_vec(2) = gbl->sres(j,m,1);
			
			int info;
			char trans[] = "T";
			GETRS(trans,3,1,&gbl->sdt2(j,m,0,0),NV+x.NV,&gbl->spiv2(j,m,0),res_vec.data(),3,info);
			if (info != 0) {
...
#else
	if (is_master) {
		/* Precondition mesh and energy side residuals */
		for(int j=0;j<base.nseg;++j) {
			int ind1 = x.npnt*vdofs +base.seg(j)*sm*x.NV +c0_indices[0];  // Index of heat equation
			for(int m=0;m<sm;++m) {
				res_vec(0) = res(ind1);
...
#endif
	{
		...
		do {
			...
			res_vec(0) = res(row+heatindex);
			res_vec(1) = res(row+x.NV);
			res_vec(2) = res(row+x.NV+1);
			
			int info;
			char trans[] = "T";
			GETRS(trans,3,1,&gbl->vdt(i,0,0),NV+x.NV,&gbl->vpiv(i,0),res_vec.data(),3,info);
			if (info != 0) {
			...
			res(row+heatindex) = res_vec(0);
			res(row+x.NV) = res_vec(1);
			res(row+x.NV+1) = res_vec(2);
		} while (++i < base.nseg);
...
void melt_cd::petsc_premultiply_jacobian() {
	const int vdofs = x.NV +(x.mmovement == tri_hp::coupled_deformable)*x.ND;
	const int heatindex = c0_indices[0];
	Array<int,1> row_ind(3);
...
#endif
				int info;
				GETRF(3,3,&gbl->sdt2(j,m,0,0),x.NV+NV,&gbl->spiv2(j,m,0),info);
				if (info != 0) {
...
#endif
	{
		...
		do {
			sind = base.seg(i);
			int row = x.seg(sind).pnt(0)*vdofs;
			row_ind(0) = row +heatindex;
			row_ind(1) = row+vdofs-2;
			row_ind(2) = row+vdofs-1;
			// Store Jacobian
...
#endif
			int info;
			GETRF(3,3,&gbl->vdt(i,0,0),x.NV+NV,&gbl->vpiv(i,0),info);
			if (info != 0) {
				*x.gbl->log << "DGETRF FAILED IN VERTEX MODE PRECONDITIONER\n";
				sim::abort(__LINE__,__FILE__,x.gbl->log);
			}
			...
			x.J.match_patterns(3, row_ind);
			x.J.combine_rows(3, row_ind, A, x.NV+NV, vpiv);
			x.J_mpi.match_patterns(3, row_ind);
			x.J_mpi.combine_rows(3, row_ind, A, x.NV+NV, vpiv);
