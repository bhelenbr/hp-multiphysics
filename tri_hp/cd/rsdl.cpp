         ...
         for(tind = 0; tind<ntri;++tind) {
            ...
            for(i=0;i<basis::tri(log2p).gpx;++i) {
               for(j=0;j<basis::tri(log2p).gpn;++j) {
                  ...
                  fluxx = RAD(crd(0)(i,j))*(cd_gbl->ax -mvel(0)(i,j))*u(0)(i,j);
                  fluxy = RAD(crd(0)(i,j))*(cd_gbl->ay -mvel(1)(i,j))*u(0)(i,j);
            ...
            if (sim::beta[stage] > 0.0) {
               ...
               for(i=0;i<basis::tri(log2p).gpx;++i) {
                  for(j=0;j<basis::tri(log2p).gpn;++j) {
                     ...
                     res(0)(i,j) -= RAD(crd(0)(i,j))*cjcb(i,j)*cd_gbl->src->f(0,pt);
                  }
               }         
               ...
               for(i=0;i<basis::tri(log2p).gpx;++i) {
                  for(j=0;j<basis::tri(log2p).gpn;++j) {
                     ...
                     cjcb(i,j) = cd_gbl->nu*RAD(crd(0)(i,j))/cjcb(i,j);
