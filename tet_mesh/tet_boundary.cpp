		...
		case(0): {
			if (is_frst()) {
				...
				for (int i=0; i<nseg; ++i) {
					FLT dist = 0.0;
					for (int n=0; n<3; ++n) {
						dist += pow(fsndbuf(count++)-x.pnts(x.seg(seg(i).gindx).pnt(0))(n), 2.0);
					}
					if (dist > 10.*EPSILON) {
						*x.gbl->log << "Matching edge numbering error after reorder" << idprefix << endl;
						sim::abort(__LINE__, __FILE__, x.gbl->log);
					}
				}
				FLT dist = 0.0;
				for (int n=0; n<3; ++n) {
					dist += pow(fsndbuf(count++)-x.pnts(x.seg(seg(nseg-1).gindx).pnt(1))(n), 2.0);
				}
				if (dist > 10.*EPSILON) {
					*x.gbl->log << "Matching edge numbering error after reorder" << idprefix << endl;
					sim::abort(__LINE__, __FILE__, x.gbl->log);
